...
#Net
      ...
      strenght="^fg($xtitle)Net: ^fg($xfg)$strenght^fg($xext)%"
...
###############################################
#Panel >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        right="$separator $date $separator^fg() $uptime $separator^fg() $mem $separator^fg() $volIco^fg($selfg) $volm^fg()$separator^fg() $strenght^fg() $separator^fg()"
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
