...
#Uptime
        ...
        uptime="^fg($xtitle)Up: ^fg($xfg)${hours}^fg($xext)h ^fg($xfg)${mins}^fg($xext)m" 
################################################
#Memory
...
###############################################
#Panel >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        right="$separator^fg() $date $separator^fg() ^i(/usr/share/icons/stlarch_icons/logout1.xbm) $uptime $separator^fg() ^i(/usr/share/icons/stlarch_icons/mem1.xbm) $mem $separator^fg() ^i(/usr/share/icons/stlarch_icons/vol1.xbm) $volm^fg $separator^fg() ^i(/usr/share/icons/stlarch_icons/wireless1.xbm) $strenght^fg()$separator^fg() $separator^fg() $separator^fg()"
